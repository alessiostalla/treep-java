...
INTERPOLATION_START: '{' -> channel(HIDDEN), pushMode(DEFAULT_MODE);
INTERPOLATION_END: '}' -> channel(HIDDEN), popMode;
DATUM: (~(' ' | '\t' | '\r' | '\n' | '(' | ')' | '\'' | '`' | ',' | '"' | '{' | '}'))+;
WS: (' ' | '\t') -> channel(HIDDEN);
...
STRING_INTERPOLATION_START: '${' -> channel(HIDDEN), pushMode(DEFAULT_MODE);
